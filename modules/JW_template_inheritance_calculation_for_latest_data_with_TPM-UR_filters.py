...
for fa in insert:
#	print fa
        out_pos = open(path_current+"Results%s/%s_insertion_trio_inheritance.txt"%(option_tag,fa),"w")
	...
	out_denovo = open(path_current+"Results%s/%s_inheritance_calculation_error_rate.txt"%(option_tag,fa),"w")  # only for child
...
#		print type
		...
		fh = open(path_current+"output_bwa-blast%s/latest_data_TPM-10UR_filters_%s_insertion_%s%sbed"%(option_tag,fa,type,option_tag),"r")
		data = fh.readlines()
		fh.close()
		for row in data:
			row = row.strip().split("\t")
...
fig.savefig(path_current+'Results%s/unrel_ind_count_loci_group_poly_and_novel_poly.pdf'%option_tag)
plt.close(fig)
...
ax.legend((rects1[0], rects2[0]), ('Polymorphic', 'Novel_polymorphic'))
fig.set_size_inches(12.5, 10.5, forward=True)
fig.savefig(path_current+'Results%s/all_ind_count_loci_group_poly_and_novel_poly.pdf'%option_tag)
plt.close(fig)
